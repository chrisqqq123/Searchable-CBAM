...
class BasicBlock(nn.Module):
    ...
    def forward(self, x, alpha):
        ...
        out = self.cbam(out) * alpha + (1-alpha) * out
...
class ResNetCBAM_search(nn.Module):
    def __init__(self, block, num_blocks, num_classes=100):
        super(ResNetCBAM_search, self).__init__()
        self.in_planes = 64
    ...
    def _build_arch_parameters(self, num_blocks):
        ...
        for num in num_blocks:
            setattr(self, alphas[i], nn.Parameter(torch.autograd.Variable(torch.zeros(num, 1).cuda(), requires_grad=True)))
            i += 1
        return {"alphas": alphas}, [getattr(self, name) for name in alphas]

def ResNetSearch18():
    return ResNetCBAM_search(BasicBlock, [2,2,2,2])

def ResNetSearch34():
    return ResNetCBAM_search(BasicBlock, [3,4,6,3])

def ResNetSearch50():
    return ResNetCBAM_search(Bottleneck, [3,4,6,3])

def ResNetSearch101():
    return ResNetCBAM_search(Bottleneck, [3,4,23,3])

def ResNetSearch152():
    return ResNetCBAM_search(Bottleneck, [3,8,36,3])