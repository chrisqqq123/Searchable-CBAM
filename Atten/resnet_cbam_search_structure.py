...
from attention_module import CBAM, ChannelAttention, SpatialAttention
...
class BasicBlock(nn.Module):
    ...
    def forward(self, x, alpha, beta):
        ...
        out = out * beta[0] + out * self.ca1(out) * beta[1] + out * self.sa1(out) * beta[2]
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        self.ca1 = ChannelAttention(self.expansion*planes)
        self.sa1 = SpatialAttention()
        # self.ca2 = ChannelAttention(self.expansion*planes)
        # self.sa2 = SpatialAttention()
...
class ResNetCBAM_search_structure(nn.Module):
    def __init__(self, block, num_blocks, num_classes=100):
        super(ResNetCBAM_search_structure, self).__init__()
        self.in_planes = 64
...
def ResNetCBAMSS18():
    return ResNetCBAM_search_structure(BasicBlock, [2,2,2,2])

def ResNetCBAMSS34():
    return ResNetCBAM_search_structure(BasicBlock, [3,4,6,3])

def ResNetCBAMSS50():
    return ResNetCBAM_search_structure(Bottleneck, [3,4,6,3])

def ResNetCBAMSS101():
    return ResNetCBAM_search_structure(Bottleneck, [3,4,23,3])

def ResNetCBAMSS152():
    return ResNetCBAM_search_structure(Bottleneck, [3,8,36,3])