...
def fix_arch(net):
    for idx, arch_name in enumerate(net._arch_names):
        for name_a in arch_name['alphas']:
            for i in range(len(getattr(net, name_a))):
                # 0 for identity, 1 for CA, 2 for SA
                a = F.softmax(getattr(net, name_a)[i])  # a size: 1*3
                # getattr(net, name_a)[i].data.copy_(torch.Tensor([float('-inf'),float(5),float('-inf'),float('-inf'),float('-inf')]).cuda())
                if abs(a[3]-a[4])<0.1 and a[3]+a[4] > max(a[0:3]):
                    getattr(net, name_a)[i].data.copy_(torch.Tensor([float('-inf'),float('-inf'),float('-inf'),float(5),float(5)]).cuda())
                elif a[0] == max(a):
                    getattr(net, name_a)[i].data.copy_(torch.Tensor([float(5),float('-inf'),float('-inf'),float('-inf'),float('-inf')]).cuda())
                elif a[1] == max(a):
                    getattr(net, name_a)[i].data.copy_(torch.Tensor([float('-inf'),float(5),float('-inf'),float('-inf'),float('-inf')]).cuda())
                elif a[2] == max(a):
                    getattr(net, name_a)[i].data.copy_(torch.Tensor([float('-inf'),float('-inf'),float(5),float('-inf'),float('-inf')]).cuda())
                elif a[3] == max(a):
                    getattr(net, name_a)[i].data.copy_(torch.Tensor([float('-inf'),float('-inf'),float('-inf'),float(5),float('-inf')]).cuda())
                elif a[4] == max(a):
                    getattr(net, name_a)[i].data.copy_(torch.Tensor([float('-inf'),float('-inf'),float('-inf'),float('-inf'),float(5)]).cuda())
        for i in range(len(net._arch_names[0]["alphas"])):
            print('Arch parameters: ',F.softmax(getattr(net, net._arch_names[0]["alphas"][i]), dim=-1) )
